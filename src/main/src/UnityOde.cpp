...
//thread local storage: every thread knows which context it is manipulating and getting data from
 static __thread int s_setIdx=ALLTHREADS;
 static __thread int s_getIdx=0;
...
*/
	...
	/*
	long UnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionPtrs)
	{
...
bool initOde(int numThreads)
{
	...
	for (int i=0; i<numThreads; i++)
	{
		...
		contexts[i].ground= dCreatePlane(contexts[i].space,0,0,1,0);
	}
	s_initialized=true;
	return true;
}
...
//odeJointSetHingeParam(3, dParamFMax, MaxForce);  //setting maximum force

//odeJointSetHingeParam(3, dParamVel,DesiredVelocity);
	...
	//	dBodyID body=c.bodies[bodyId];
		...
		dReal side2[3] = {.6,.6,0.05};

		OdeThreadContext & c=contexts[s_getIdx];

dJointID odeJoint = contexts[threadIdx].joints[3];
...
//odeBodyAddTorque(2,torque);
 //const dReal angle = odeJointGetHingeAngle(3);
	//const dReal *P =  dBodyGetPosition(c.bodies[5]);
 //printf("IN LOOP x: %f, y:%f, z: %f \n", P[0],P[1],P[2]);
	 // printf("hinge angle : %f", angle);

	//  dVector3 torque={0,.02,0};
		//odeBodyAddTorque(2,torque);




	//dsDrawBox( dBodyGetPosition(c.bodies[2]),dBodyGetRotation(c.bodies[2]),side2);
	 //  dsDrawSphere( dBodyGetPosition(c.bodies[5]),dBodyGetRotation(c.bodies[5]),0.03f);
...
{
	OdeThreadContext &context = contexts[s_getIdx];
	AALTO_ASSERT1(context.contacts.find(index) != context.contacts.end());
...
bool EXPORT_API saveOdeState(int slot, int sourceContext)
{
	...
	for(i = contexts[sourceContext].bodies.begin(); i != contexts[sourceContext].bodies.end(); ++i){
		...
		const dReal *inpos = dBodyGetPosition(odeBody);
		const dReal *inlinvel = dBodyGetLinearVel(odeBody);
		const dReal *inangvel = dBodyGetAngularVel(odeBody);
		const dReal *inquaternion = dBodyGetQuaternion(odeBody);
/*
		printf("inside x %f \n", inpos[0]);
		printf("inside lin x %f \n", inlinvel[0]);
		printf("inside angular  x %f \n", inangvel[0]);
		printf("inside quaternion  x %f \n", inquaternion[0]);
		*/
	...
	for(ji = contexts[sourceContext].joints.begin(); ji != contexts[sourceContext].joints.end(); ++ji){
		...
		case dJointTypeHinge:
			...
			//printf("hinge param %f",save.joints[saveIdx].fmax[0] );
			break;
...
bool EXPORT_API restoreOdeState(int slot, bool breakOnErrors)
{
	...
	ITERATE_THREADS(threadIdx)
	{
		...
		for(i = c.bodies.begin(); i != c.bodies.end(); ++i){
			dBodyID odeBody=i->second;
			if(!odeBody) continue;
        //printf("from save state %f \n", save.bodies[saveIdx].angularVelocity[0]);
...
{
	AALTO_ASSERT1(threadIdx<(int)contexts.size());
	s_setIdx=threadIdx;
	if (threadIdx<0)
		s_getIdx=0;
	else
...
EXPORT_API void odeJointSetFmax(int jointId, float fmax1, float fmax2, float fmax3)
{
	ITERATE_THREADS(threadIdx)
	{
		...
		case dJointTypeAMotor:
			...
			break;
		default:
			Debug::throwError("odeJointSetFMax: unsupported joint type");
		}
	}
...
EXPORT_API void odeJointGetMotorAnglesDegrees(int jointId, OdeVector result)
{
	...
	case dJointTypeAMotor:
		...
		break;
	default:
		Debug::throwError("odeJointGetMotorAngles: unsupported joint type");
	}
}
...
EXPORT_API void setMotorVelocities(int nJoints, int *jointIds, const float *velocities)
{
	...
	for (int i=0; i<nJoints; i++)
	{
		...
		default:
			Debug::throwError("setMotorVelocities: unsupported joint type");
		}
	}
}
