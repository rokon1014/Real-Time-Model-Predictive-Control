...
{	
	...
	if ( result_ball1[0] > minX && result_ball1[0]< maxX && result_ball1[1] > minY && result_ball1[1] <maxY  )
        {
         ...
         new1Y = (result_ball1[0] - minX) * per_pixelX;
         new1X = (result_ball1[1]- minY) * per_pixelY ;

        //printf( " per_pixelY %f, per_pixelX %f  ref X %d\n", per_pixelY, per_pixelX,diffX);
        //printf("ball 1 x = %f,y=%f\n", (new1X-23.0)/100.0, (new1Y-18.0)/100.0); 
	...
	if ( result_ball2[0] > minX && result_ball2[0]< maxX && result_ball2[1] > minY && result_ball2[1] <maxY  )
        {
         ...
         new2Y = (result_ball2[0] - minX) * per_pixelX;
         new2X = (result_ball2[1]- minY) * per_pixelY ;

        //printf( " per_pixelY %f, per_pixelX %f  ref X %d\n", per_pixelY, per_pixelX,diffX);
        //printf("ball 2 x = %f,y=%f\n", (new2X-23.0)/100.0, (new2Y-18.0)/100.0); 
	...
	app::twoBall cam_poss ;
	cam_poss.pos1x = new1X;
    cam_poss.pos1y = new1Y ;

    cam_poss.pos2x = new2X;
    cam_poss.pos2y = new2Y ;

    //cam_posX.data = newX;
    //cam_posY.data = newY;
  //  printf("pos1x %f\n", cam_poss.pos1x);

    chatter_pubX.publish(cam_poss);
...
int main(int argc, char **argv)
{
	...
	ball1.window = "ball1_window";
	ball2.window = "ball2_window";
...
//Ball 2
	...
	ros::spinOnce();
    pub = it.advertise("camera/image_processed", 1);
    chatter_pubX = nh.advertise <app::twoBall>("/from_cameraXX", 1);
