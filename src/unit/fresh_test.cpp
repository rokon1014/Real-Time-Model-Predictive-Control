...
float minControl=-1.0;	//lower sampling bound
float maxControl=1.0;	//upper sampling bound
...
bool robot()
{
...

	{
...

		{
			...
			float cost=squared((pos[0])*20.0f)+ squared(angle*5.0f) ;
	...
	odeJointSetHingeParam(mainLink.joint,dParamVel,-0.1);
	printf("positive 0.1\n");
}
...
int main(int argc, char **argv)
{
	...
	odeBodySetPosition(ball.body,0,0.00,h_floor_table+h_base+h_sphere+h_support+stage_dim[2]);
