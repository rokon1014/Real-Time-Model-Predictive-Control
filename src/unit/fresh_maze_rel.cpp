...
float C=1.0;
float controlStd[2]={0.5f*C,0.5f*C}; // 0.65 both working //sqrt(\sigma_{0}^2 C_u) of the paper (we're not explicitly specifying C_u as u is a scalar here). In effect, a "tolerance" for torque minimization in this test
float controlDiffStd[2]={10.8f*C, 10.8f*C}; // 0.9 both working //sqrt(\sigma_{1}^2 C_u) in the pape. In effect, a "tolerance" for angular jerk minimization in this test
float controlDiffDiffStd[2]={15.0f*C,15.0f*C}; //18.5 both working//sqrt(\sigma_{2}^2 C_u) in the paper. A large value to have no effect in this test.
...
///////////////////////////////////////////////////////////////////
	...
	//signal the start of a planning step
		...
		for (int i=0; i<nSamples; i++)
		{
			...
			float cost=squared((0.06-rel)*12.5f)+squared((rel_second)*12.5f)+squared(angle*10.5)+squared(angle_second*10.5)+squared(vel_inside[0]*(0.09-rel)*20.5f) + squared(vel_inside[1]*(rel_second)*20.5f);//+squared(control[0]*1.0f)+ squared(control[1]*1.0f) ;
			//+squared(control[0]*1.5)+squared(control[1]*1.5) ;//+ squared(vel_robotX*0.05f)+ squared(vel_robotY*0.05f) ;
		//	if (-0.06<rel_second && rel_second <0.06 && -0.02<rel && rel<-0.09 )
		//	{
		//	cost = cost+700;
		//	}
...
//creating Obstacle cone first part
	...
	odeMassSetBoxTotal(obs.body,0.002,0.031,0.031,0.02 );
	odeBodySetPosition(obs.body,0.045,0.043,h_floor_table+h_base+h_sphere+h_support+stage_dim[2]+obs.radius/2);
	odeGeomSetBody(obs.geom,obs.body);
	printf("capsule obstacle body id %f, geom id %f \n", obs.body, obs.geom);
	...
	odeBodySetPosition(cone.body,0.045,-0.043,h_floor_table+h_base+h_sphere+h_support+stage_dim[2]+obs.radius/2);
	...
	odeBodySetPosition(obs2.body,0.04,0.0,h_floor_table+h_base+h_sphere+h_support+stage_dim[2]+obs.radius/2);
