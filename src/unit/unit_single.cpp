...
//init all trajectories to the master state
...

		{
			...
			for (int i=0; i<nSamples; i++)
			{
			 ...
			 float cost=squared((0.01-pos[0])*200.0f) +squared(ball_vel*0.3)+squared(control * 0.3) ; //+ squared(ball_vel*0.2)+ squared(angle*0.3f)  ;
	   ...
	   ROS_INFO("dt %f", dt);
		...
		//final << std::setw(12)<<" best control" <<std::setw(12)<<control << std::setw(12)<<"best cost" <<std::setw(12)<< cost << std::setw(12)<< "position" << std::setw(12)<< last_position<<std::setw(12)<<last_position <<endl ;
...
/*
	...
	*/
    ...
    odeBodySetPosition(body1,0.1,0,1.7f);
	//odeBodySetMass (body1, 0.04);
